...
try:
    ...
    KerberosTicket = None
    logger.warning(
        "krtc not installed: Kerberos ticket-based authentication unavailable."
    )
...
class QuestionnaireClient:
    ...
    def __init__(self, url=None, use_kerberos=True, user=None, pw=None):
        if use_kerberos:
            ...
            self.krbheaders = KerberosTicket(
                "HTTP@" + urlparse(self.questionnaire_url).hostname
            ).getAuthHeaders()
    ...
    def getEnumerations(self, run):
        ...
        """
        r = self.rget(
            self.questionnaire_url + "ws/questionnaire/" + run + "/get_enum_field_names"
        )
    ...
    def getProposalsListForRun(self, run):
        ...
        if r.status_code <= 299:
            ...
            for experiment in experiments["experiments"]:
                proposals[experiment["proposal_id"]] = {
    ...
    def _updateIfExists(self, ret, srcdata, srcField, destName):
        ...
        :param: srcField - The srcField in canonical format - for example,
                contacts.point_of_contact
        :param: destName - The name of the field in ret. So we can take
                contacts.point_of_contact and create a POC field..
        """
        data = srcdata
    ...
    def getProposalDetailsForRun(self, run, proposalid):
        ...
        r = self.rget(
            self.questionnaire_url + "ws/proposal/attribute/" + run + "/" + proposalid
        )
        if r.status_code <= 299:
            proposalData = r.json()
            ...
            ret.update(
                {
                    x["id"]: x["val"]
                    for x in [
                        item for sublist in proposalData.values() for item in sublist
                    ]
                }
            )
        ...
        if r.status_code <= 299:
            ...
            self._updateIfExists(
                ret, urawiData, "contacts.point_of_contact", "urawi_poc"
            )
            ...
            self._updateIfExists(
                ret, urawiData, "info.spokesPerson.firstName", "Spokesperson First"
            )
            self._updateIfExists(
                ret, urawiData, "info.spokesPerson.lastName", "Spokesperson Last"
            )
            self._updateIfExists(
                ret, urawiData, "info.spokesPerson.email", "Spokesperson Email"
            )
            self._updateIfExists(
                ret, urawiData, "info.nonURAWI_proposal", "nonURAWI_proposal"
            )
    ...
    def formLabelMappings(self, run):
        ...
        This call generates a dict with the mapping between the attribute value
        name and the reporting label if one exists.
        This is also the process used to map multiple values into and array
        under a single reporting label.
        For example, xraytech-tech-1, xraytech-tech-2 etc will be mapped to
        "X-ray Techniques" as an array
        """
        nameMappings = {}
        tabNames = self.rget(
            self.questionnaire_url + "ws/questionnaire/" + run + "/tabnames"
        ).json()
        for formTabName in tabNames:
            ...
            r = self.rget(
                ...
                + "ws/questionnaire/"
                + run
                + "/form_data_definitions?form_name="
                + formTabName
            )
            if r.status_code <= 299:
                ...
                for formDefinition in formDefinitions:
                    if "reporting_label" in formDefinition:
                        nameMappings[formDefinition["attribute_id"]] = formDefinition[
                            "reporting_label"
                        ]
                    else:
                        if (
                            "quantity" in formDefinition
                            and int(formDefinition["quantity"]) > 1
                        ):
                            nameMappings[
                                formDefinition["attribute_id"]
                            ] = formDefinition["attribute_id"][
                                : (formDefinition["attribute_id"].rfind("_") - 1)
                            ]
            else:
                raise Exception("Invalid HTTP status code from server", r.status_code)
        return nameMappings
    ...
    def getProposalsStatusForRun(self, run):
        ...
        """
        r = self.rget(
            self.questionnaire_url + "ws/questionnaire/proposals_status/" + run
        )
        if r.status_code <= 299:
    ...
    def getProposalsPersonnelForRun(self, run):
        ...
        """
        r = self.rget(
            self.questionnaire_url + "ws/questionnaire/proposals_personnel/" + run
        )
            ...
            for data in datas:
                if data["startDate"] and data["endDate"]:
                    stdate = datetime.datetime.strptime(
                        data["startDate"], "%Y-%m-%d %H:%M:%S"
                    )
                    enddate = datetime.datetime.strptime(
                        data["endDate"], "%Y-%m-%d %H:%M:%S"
                    )
                    data["daysToStart"] = (stdate - now).days
                    data["daysToEnd"] = (enddate - now).days
                    logger.debug(
                        "Start date %s End date %s daysToStart %s daysToEnd %s",
                        data["startDate"],
                        data["endDate"],
                        data["daysToStart"],
                        data["daysToEnd"],
    ...
    def updateProposalAttribute(self, run, proposal_id, attrname, attrvalue):
        """
        Updates the attribute specified by attrname to the value specified by
        attrvalue for the proposal specified by proposal_id in the specified
        run.

        Writes to the questionnaire do require authentication/authorization; so
        it's best to use Kerberos for this.

        For example:

        .. code:: python

            qscli.updateProposalAttribute(
                "run17",
                "LR63",
                "pcdssetup-motors-setup-1-purpose",
                "Value of purpose"
            )
        """
        r = self.rpost(
            self.questionnaire_url + "ws/proposal/attribute/" + run + "/" + proposal_id,
            data={"run_id": run, "id": attrname, "val": attrvalue},
        )
        if r.status_code <= 299:
            return r.json()
        else:
